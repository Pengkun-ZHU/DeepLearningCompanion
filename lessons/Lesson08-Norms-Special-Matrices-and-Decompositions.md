...
Understanding their computational cost helps you reason about where they should and should not appear in a training loop.
...
where `λ` are eigenvalues and the columns of `V` are eigenvectors.
...
SVD in particular scales as O(min(m,n) * m * n) for an m×n matrix.
